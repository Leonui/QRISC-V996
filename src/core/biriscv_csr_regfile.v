...
begin
    ...
    // FIX: mip.SEIP/MEIP 是电平位,应跟随中断控制器输出(ext_intr_i),不能粘住。
    // 原逻辑只 OR、从不清除 -> SEIP 一旦置位永久为 1,内核又写不了它(只读),
    // 导致外部中断(如 uartlite 经 Xilinx INTC)只能触发一次,之后处理器陷在
    // 无效中断里、真正的后续中断进不来。这里在 OR 之前清掉这两个外部中断位,
    // 让它们每周期从 ext_intr_i 重新求值(电平跟随)。timer/软件中断位不动。
    csr_mip_r[`SR_IP_MEIP_R] = 1'b0;
    csr_mip_r[`SR_IP_SEIP_R] = 1'b0;
